...
export const updateProfile = async (req, res) => {
  try {
    const allowed = ['name', 'dateOfBirth', 'gender', 'diagnosis', 'medications', 'language', 'timezone', 'notificationsEnabled'];
    const updates = {};
    allowed.forEach(f => { if (req.body[f] !== undefined) updates[f] = req.body[f]; });
    const patient = await Patient.findByIdAndUpdate(req.userId, updates, { new: true, runValidators: true }).select('-password');
    res.json({ success: true, data: patient });
  } catch {
...
export const createMedication = async (req, res) => {
  try {
    ...
    if (!name || !dosage || !startDate)
      return res.status(400).json({ success: false, error: 'name, dosage and startDate are required' });
...
export const deleteMedication = async (req, res) => {
  try {
    await Medication.findOneAndDelete({ _id: req.params.id, patient: req.userId });
...
export const getNotificationSettings = async (req, res) => {
  try {
    ...
    res.json({ success: true, data: settings || { reminderTime: '20:00', reminderEnabled: true, reminderDays: [0,1,2,3,4,5,6] } });
  } catch {
